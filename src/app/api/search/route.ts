...
export async function GET(request: Request) {
  ...
  try {
    ...
    // 限制并发请求数量，可通过环境变量 SEARCH_CONCURRENCY 控制，默认为8
    const searchConcurrency = (() => {
      const envValue = process.env.SEARCH_CONCURRENCY;
      if (envValue) {
        const parsed = parseInt(envValue, 10);
        if (!isNaN(parsed) && parsed > 0) {
          console.log(`搜索并发数已通过环境变量设置为: ${parsed}`);
          return parsed;
        }
        console.warn(`环境变量 SEARCH_CONCURRENCY 的值 "${envValue}" 不是有效的正整数，使用默认值 8`);
      }
      console.log('搜索并发数使用默认值: 8');
      return 8;
    })();
